					...
					<div class="p-4">
						<div class="flex items-start">
							<div class="flex-shrink-0">
								<component
									:is="icon"
									:class="['h-6 w-6', isError ? 'text-red-400' : 'text-green-400']"
									aria-hidden="true" />
							</div>
							<div class="ml-3 w-0 flex-1 pt-0.5">
								...
								<p class="mt-1 text-sm text-gray-500" v-if="showSubTitle">
									{{subTitle}}
								</p>
							</div>
...
const icon = ref(CheckCircleIcon);
const title = computed(() => props.title);
const subTitle = computed(() => props.subTitle);
const showSubTitle = computed(() => props.showSubTitle)
...
onMounted(() => {
	isError.value ? (icon.value = XCircleIcon) : (icon.value = CheckCircleIcon);
});
</script>
